---
title: 前端中的IoC理念
date: 2019-01-04 17:56:47
tags: [IoC,依赖注入,控制反转,面向接口编程]
---

[原文](https://juejin.im/post/5c2c47dcf265da616d544a53) 
...
概念总是抽象的，所以下面将以一个例子来解释上述的概念。假设需要构建一款应用叫 `App`，它包含一个路由模块 `Router` 和一个页面监控模块 `Track`，一开始可能会这么实现：
...
所谓的依赖注入，简单来说就是把高层模块所依赖的模块**通过传参的方式**把依赖「**注入**」到模块内部，上面的代码可以通过依赖注入的方式改造成如下方式：
...
虽然 `App` 通过依赖注入的方式在一定程度上解耦了与其他几个模块的依赖关系，但是还不够彻底，其中的 `this.router` 和 `this.track` 等属性其实都**还是对「具体实现」的依赖**，明显违背了 `IoC` 思想的准则，那如何进一步抽象 `App` 模块呢。
...
class App {
    ...
    initModules() {
        App.modules.map(
            module => module.init && typeof module.init == 'function' && module.init(this)
        );
    }
}
...
```
 ...
 经过改造后 `App` 内**已经没有「具体实现」了**，看不到任何业务代码了，那么如何使用 `App` 来管理我们的依赖呢： 
...
可以发现 `App` 模块在使用上也非常的方便，**通过 `App.use()` 方法来「注入」依赖**，在 `./modules/some-module.js` 中**按照一定的「约定」去初始化相关配置**，比如此时需要新增一个 `Share` 模块的话，无需到 `App` 内部去修改内容：
